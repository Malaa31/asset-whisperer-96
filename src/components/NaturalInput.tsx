 ...
 */
export function NaturalInput({ onParsed }: { onParsed: (p: ParsedAsset) => void }) {
  const [text, setText] = useState("");
  const [listening, setListening] = useState(false);
  const [micError, setMicError] = useState("");
  // Détecté après montage : `window` n'existe pas au rendu serveur, et
  // l'évaluer pendant le rendu figeait le bouton à « absent ».
        ...
        <div className="mt-3 rounded-xl border border-border bg-card p-3">
          ...
          <p className="mt-1 text-[11px] leading-relaxed text-muted-foreground">
            {preview.summary.length
              ...
              : "Rien de reconnu pour l'instant."}
          </p>
        </div>
      )}
