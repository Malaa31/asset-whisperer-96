...
import { A as ChevronUp, F as Bitcoin, L as Banknote, M as ChevronDown, O as CreditCard, R as ArrowRight, S as Landmark, b as Mic, d as ShieldCheck, f as Search, g as PiggyBank, h as Plus, i as Upload, j as ChevronRight, l as Square, m as RefreshCw, n as Wallet, r as User, s as Trash2, t as X, u as Sparkles, v as Package, w as House, x as LoaderCircle, z as ArrowLeft } from "../_libs/lucide-react.mjs";
...
//#endregion
//#region node_modules/.nitro/vite/services/ssr/assets/router-zm3Pr5My.js
...
var styles_default = "/assets/styles-D-Sooo1I.css";
...
var $$splitComponentImporter$2 = () => import("./routes-DuGC9y15.mjs");
...
var $$splitComponentImporter$1 = () => import("./patrimoine-uBpy8jgo.mjs");
var Route$5 = createFileRoute("/patrimoine")({
	head: () => ({ meta: [
...
var $$splitComponentImporter = () => import("./profil-DmC9R79a.mjs");
...
var Route$3 = createFileRoute("/api/public/fx")({ server: { handlers: { GET: async ({ request }) => {
...
var Route$2 = createFileRoute("/api/public/history")({ server: { handlers: { GET: async ({ request }) => {
	if (!isAllowedOrigin(request)) return forbidden();
	const symbol = (new URL(request.url).searchParams.get("symbol") ?? "").trim().toUpperCase();
	if (!/^[A-Z0-9.^-]{1,20}$/.test(symbol)) return Response.json({ error: "invalid_symbol" }, { status: 400 });
	try {
		const res = await fetch(`https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(symbol)}?range=max&interval=1mo&events=none`, {
			headers: {
		...
		for (let i = 0; i < stamps.length; i++) {
			const c = closes[i];
			const t = stamps[i];
			if (typeof c === "number" && Number.isFinite(c) && c > 0 && typeof t === "number") points.push({
				t: t * 1e3,
				c
			});
		}
...
var rootRouteChildren = {
	...
	PatrimoineRoute: Route$5.update({
		id: "/patrimoine",
		path: "/patrimoine",
		getParentRoute: () => Route$7
	}),
	...
	ApiPublicFxRoute: Route$3.update({
		id: "/api/public/fx",
		path: "/api/public/fx",
		getParentRoute: () => Route$7
	}),
	ApiPublicHistoryRoute: Route$2.update({
